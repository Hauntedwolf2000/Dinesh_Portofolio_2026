...
type Section = 'projects' | 'skills' | 'experience' | 'certifications' | 'achievements' | 'testimonials' | 'about' | 'resume'
...
export async function GET(req: NextRequest) {
  ...
  if (section === 'resume') return NextResponse.json({ resumeUrl: data.resumeUrl })
  if (section && section in data) return NextResponse.json((data as Record<string, unknown>)[section])
...
export async function POST(req: NextRequest) {
  ...
  const { section, item } = body as { section: Section; item: Record<string, unknown> }
  ...
  const arr = (data as Record<string, unknown[]>)[section] as Record<string, unknown>[]
...
export async function DELETE(req: NextRequest) {
  ...
  if (!section || !id) return NextResponse.json({ error: 'section and id required' }, { status: 400 })
